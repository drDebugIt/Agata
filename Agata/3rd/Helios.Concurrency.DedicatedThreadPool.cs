...
namespace Helios.Concurrency
{
    ...
    internal sealed class DedicatedThreadPool : IDisposable
    {
        public DedicatedThreadPool(DedicatedThreadPoolSettings settings)
        {
            ...
            Settings = settings;
            _workers = Enumerable.Range(1, settings.NumThreads).Select(workerId => new PoolWorker(this, workerId))
                .ToArray();
        ...
        public bool QueueUserWorkItem(IAction work)
        {
            if (work == null)
            ...
            private void RunThread()
            {
                try
                {
                    foreach (var action in _pool._workQueue.GetConsumingEnumerable())
                    {
                        ...
                        {
                            if (action is ThreadPoolSystemActionWrapper wrapper)
                            {
                                ThreadPoolSystemActionWrapper.Release(wrapper);
                            }
                        }
                    }
                }
        ...
        private class ThreadPoolWorkQueue
        {
            ...
            private readonly ConcurrentQueue<IAction> _queue = new ConcurrentQueue<IAction>();
            ...
            public bool TryAdd(IAction work)
            {
            ...
            public IEnumerable<IAction> GetConsumingEnumerable()
            {
                while (true)
                {
                    IAction work;
...
#pragma warning restore 169
            ...
            {
                if (Interlocked.CompareExchange(ref m_state.RawData, newState.RawData, currentState.RawData) ==
